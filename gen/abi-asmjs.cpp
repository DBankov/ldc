...
struct AsmJSTargetABI : TargetABI {
  ...
  bool returnInArg(TypeFunction *tf) override {
    ...
    // all others via sret
    return !canRewriteAsInt(rt, false);
  }
  ...
  void rewriteFunctionType(TypeFunction *tf, IrFuncTy &fty) override {
    ...
    if (!fty.ret->byref) {
      ...
      if (isAggregate(rt) && !isMagicCppStruct(rt) && canRewriteAsInt(rt, false) &&
...
TargetABI *getAsmJSTargetABI() { return new AsmJSTargetABI(); }
